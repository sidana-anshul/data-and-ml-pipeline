...
from employees_cap
where first_name = 'Hercules' and regexp_like(last_name, '^B.*');
...
join dept_emp_cap t2 on t1.emp_no = t2.emp_no
join departments_cap t3 on t2.dept_no = t3.dept_no
where t3.dept_name = 'Sales' limit 100;
...
join dept_emp_cap t2 on t1.emp_no = t2.emp_no
join departments_cap t3 on t2.dept_no = t3.dept_no
where t3.dept_name in ('Sales', 'development') limit 100;
...
select concat_ws('-', cast(salary as string), cast(salary+10000 as string))
from salaries_cap;

select salary, concat_ws('-', cast((floor(salary/10000)*10000) as string), cast((ceil(salary/10000)*10000) as string))
from salaries_cap limit 100;
...
max(last_date) 2013-03-10

select emp_no,
case
when left_company = 1 then datediff(last_date, hire_date)
else datediff('2013-03-10', hire_date)
